...
x0 = [0.3 0.5 0.5 0.4 0.8];
t_end = [0.2 0.15 0.012 0.035 0.012];
rhoL = [1 1 1 5.99924 1];
uL = [0.75 -2 0 19.5975 -19.59745];
pL = [1 0.4 1000 460.894 1000];
rhoR = [0.125 1 1 5.99242 1];
uR = [0 2 0 -6.19633 -19.59745];
pR = [0.1 0.4 0.01 46.0950 0.01];
...
rho = ones(size(xx));
p = rho*pL(Test_number);
v = rho*0;
u = rho*uL(Test_number);
rho = rho*rhoL(Test_number);

x0pos = ceil(x0(Test_number)/dx);

rho(:,x0pos:end)= rhoR(Test_number);
p(:,x0pos:end) = pR(Test_number);
u(:,x0pos:end) = uR(Test_number);
